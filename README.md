...
從目前 Git 使用者的 commit 歷史，整理成簡潔的摘要報告（預設過去兩週）。
...
```bash
npx skills add <your-github-username>/my-commit-report
```

全域安裝：

```bash
npx skills add -g <your-github-username>/my-commit-report
```
...
cp -R /path/to/my-commit-report/.agents/skills/my-commit-report .agents/skills/

# 或使用者層級（Cursor）
mkdir -p ~/.cursor/skills
cp -R /path/to/my-commit-report/.agents/skills/my-commit-report ~/.cursor/skills/
```
